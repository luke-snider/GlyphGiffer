...
class _LSModalWindow(vanilla.Window):
    
    nsWindowLevel = NSModalPanelWindowLevel 
    
    def __init__(self, *args, **kwargs):
        super(_LSModalWindow, self).__init__(*args, **kwargs)
    ...
    def open(self):
        super(_LSModalWindow, self).open()
        self.center()
        NSApp().runModalForWindow_(self._window)
        
    def windowWillClose_(self, notification):
        super(_LSModalWindow, self).windowWillClose_(notification)
        NSApp().stopModal()
...
class _AskStringController(_baseWindowController):
    ...
    def __init__(self, message, value, title):
        self.w = _LSModalWindow((370, 110), title)
...
class _listController(_baseWindowController):
    ...
    def __init__(self, items, message, title, showSearch=False):
        ...
        self.w = _LSModalWindow((350, 300), title)
        y = 10
